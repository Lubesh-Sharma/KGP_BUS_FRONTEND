...
function UserManagement({ user }) {
    ...
    if (user && user.token && user.role === 'admin') {
      const fetchUsers = async () => {
        try {
          ...
          const apiUrl = getApiUrl('/admin/users');
  ...
  return (
    <div className="user-management">
      ...
      <div className="user-stats" style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
        <div className="stat-box"><strong>Total:</strong> {stats.totalUsers}</div>
        <div className="stat-box"><strong>Admins:</strong> {stats.adminCount}</div>
        <div className="stat-box"><strong>Drivers:</strong> {stats.driverCount}</div>
        <div className="stat-box"><strong>Users:</strong> {stats.regularUsers}</div>
      </div>
