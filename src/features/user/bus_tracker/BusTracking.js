...
const LocationButton = ({ setUserLocation, autoFollow, setAutoFollow, userLocation, busLocation }) => {
  ...
  useEffect(() => {
    ...
    const handleError = (e) => {
      console.warn("User location watch warning:", e.message);
    };
...
const BusTracking = ({ userLocation, setUserLocation, hideDropdown = false, selectedBus = null, onDrawingRoutes }) => {
  ...
  // Real-time Socket.io listener for live bus movement
  useEffect(() => {
    if (!selectedBusState?.id) return;
    ...
    socket.emit('subscribe_bus', selectedBusState.id);
    socket.emit('join_room', 'active_buses');

    socket.on('bus_location_changed', (data) => {
      if (data && String(data.busId) === String(selectedBusState.id)) {
    ...
    socket.on('trip_status_changed', (data) => {
      if (data && String(data.busId) === String(selectedBusState.id)) {
        setIsTripCancelled(Boolean(data.isCancelled));
  ...
  const fetchBusData = async (busId, showLoading = true, refreshDataOnly = false) => {
    try {
      ...
      if (locationResponse.data && locationResponse.data.data) {
        ...
        // Check if bus is running (has valid location data)
        const locationTime = new Date(location.timestamp);
        const now = new Date();
        const timeDiffMinutes = Math.abs(now.getTime() - locationTime.getTime()) / (1000 * 60);

        // Consider bus as running if location data exists and is valid
        setBusRunning(Boolean(location.latitude && location.longitude && (isNaN(timeDiffMinutes) || timeDiffMinutes < 720)));
      } else {
      ...
      if (!refreshDataOnly || !routeDataRef.current) {
        let routeData = null;
        try {
          const cached = sessionStorage.getItem(`kgp_bus_route_${busId}`);
          if (cached) {
        ...
        if (!routeData) {
          ...
          if (routeResponse.data && routeResponse.data.data) {
            routeData = routeResponse.data.data;
            try {
              sessionStorage.setItem(`kgp_bus_route_${busId}`, JSON.stringify(routeData));
            } catch (e) {}
          }
        }
        ...
        if (routeData) {
          ...
          setIsTripCancelled(Boolean(routeData.is_cancelled || routeData.bus?.is_cancelled));
          setCancelledReason(routeData.cancelled_reason || routeData.bus?.cancelled_reason || null);
  ...
  return (
    <div className="bus-tracking">
      ...
      <div className="location-panel">
        ...
        {/* Bus trip cancelled banner */}
        {selectedBusState && isTripCancelled && (
          <div className="cancelled-trip-banner" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div>
                <strong>TRIP CANCELLED TODAY</strong>
                <p style={{ margin: '2px 0 0 0', fontSize: '12px', fontWeight: 'normal', color: '#434343' }}>
                  {cancelledReason || 'This scheduled bus departure has been marked as cancelled.'}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="map-container">
        ...
        >
          ...
          {(() => {
            if (!selectedBusState || !busLocation) return null;

            // Extract route stop coordinates for strict road-snapping
            const routePolyline = (busStops && busStops.length >= 2)
              ? busStops.map(s => [parseFloat(s.latitude), parseFloat(s.longitude)])
              : [];

            // Strictly snap bus location onto the campus road network
            const displayBusLocation = (routePolyline.length >= 2)
              ? snapToRoutePolyline(busLocation, routePolyline)
              : busLocation;
                ...
                >
                  <Popup>
                    <div className="bus-popup">
                      ...
                      <p style={{ margin: '2px 0 6px 0', fontSize: '11px', color: '#2e7d32', fontWeight: 'bold' }}>
                        📍 Snapped to Campus Road Network
                      </p>
                      {busInfo && (
                        <>
